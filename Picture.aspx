...
<asp:ListView ID="ListView1" runat="server" DataKeyNames="PostID" DataSourceID="SqlDataSource1">
        ...
        <ItemTemplate>
            <span style="">
                <asp:HyperLink ID="PictureTitleLink" Text='<%# Eval("PostTitle") %>' NavigateUrl='<%# "PictureDetail.aspx?PostID=" + Eval("PostID") + "&PostTitle=" + Server.UrlEncode(Eval("PostTitle").ToString())%>'  runat="server"></asp:HyperLink>
            <br />
            <asp:Image ID="PictureImage" Width="70" Height="70" ImageUrl='<%# Eval("[PostPicture]") %>' runat="server" />
            <br />
            <asp:Label ID="PictureDate" runat="server" Text='<%# Eval("PostDate") %>' ForeColor="Gray" />
            <br />
            <asp:Label ID="PictureSummary" runat="server" Text='<%# Eval("PictureDES") %>' />
                <br />
                <br />
            </span>
        </ItemTemplate>
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
        ...
        SelectCommand="SELECT Post.PostID, Post.PostType, Post.PostTitle, Post.PostDate, Post.PostPicture, Picture.PostIDfk, Picture.PictureDES FROM Picture INNER JOIN Post ON Picture.PostIDfk = Post.PostID ORDER BY Post.PostID DESC">
    </asp:SqlDataSource>

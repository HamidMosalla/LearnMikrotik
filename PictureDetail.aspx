<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="PictureDetail.aspx.cs" Inherits="PictureDetail" %>
...
<h2 style=" font-size:large; border-bottom:2px solid #CCC" id="PictureDetailHeader" runat="server"></h2>
<br />
<p id="PictureDetailDate" style="color:#73A" runat="server"></p> 

<p id="PictureDetailSummary" style="color:Silver" runat="server"></p>
<hr />
<br />
<br />


<asp:ListView ID="ShowPictureListView" runat="server" DataKeyNames="PostID" DataSourceID="ShowPictureSqlDataSource">
       
        <ItemTemplate>
            <span style="">
            <a href='<%# Eval("[PictureURL]") %>' style="text-align:center;font-weight:bold;color:Blue;">دیدن عکس با اندازه ی واقعی</a>
            <br />
            <br />
            <asp:Image ID="ArticleImage"  Height="400" Width="650" ImageUrl='<%# Eval("[PictureURL]") %>' runat="server" />
            </span>
        </ItemTemplate>
    ...
    <asp:SqlDataSource ID="ShowPictureSqlDataSource" runat="server" 
        ConnectionString="<%$ ConnectionStrings:LearnMikrotikConnectionString %>" 

        
        SelectCommand="SELECT Post.PostID, Picture.PostIDfk, PicturesURL.PicturePostIDfk, PicturesURL.PictureURL FROM Post INNER JOIN Picture ON Post.PostID = Picture.PostIDfk INNER JOIN PicturesURL ON Picture.PostIDfk = PicturesURL.PicturePostIDfk WHERE ([PostIDfk] = @PostIDfk)">
        <SelectParameters>
...
<hr />
    ...
    <ajaxToolkit:Rating ID="PictureRating" runat="server"
    CurrentRating="0"
    MaxRating="10"
    StarCssClass="ratingStar"
    WaitingStarCssClass="savedRatingStar"
    FilledStarCssClass="filledRatingStar"
    EmptyStarCssClass="emptyRatingStar" onchanged="PictureRating_Changed" onload="PictureRating_Load" /> 
    ...
    <asp:ListView ID="CommentList" runat="server"  
            DataSourceID="PictureCommentsDataSource" DataKeyNames="CommentID" 
    ...
    <asp:SqlDataSource ID="PictureCommentsDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:LearnMikrotikConnectionString %>" 
...
<h4 style="background-color:Blue; color:Aqua">آخرین مقالات</h4>
 <asp:ListView ID="ArticleListView" runat="server" DataKeyNames="PostID" DataSourceID="ArticleSqlDataSource">
    ...
    <asp:ListView ID="ListView1" runat="server" DataKeyNames="PostID" DataSourceID="PictureSqlDataSource">

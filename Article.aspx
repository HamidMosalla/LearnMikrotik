...
<asp:ListView ID="ListView1" runat="server" DataKeyNames="PostID" DataSourceID="SqlDataSource1">
        ...
        <ItemTemplate>
            <span style="">
                 <%-- you can use it like that too NavigateUrl='<%# Eval("NewsID", "NewsDetail.aspx?NewsID={0}") %>' --%>
               <%-- and if you don't prefix a url with http it's always gonna be relative, mmkay?--%>
                <asp:HyperLink ID="ArticleTitleLink" Text='<%# Eval("PostTitle") %>' NavigateUrl='<%# "ArticleDetail.aspx?PostID=" + Eval("PostID") + "&PostTitle=" + Server.UrlEncode(Eval("PostTitle").ToString())%>'  runat="server"></asp:HyperLink>
            <br />
             <asp:Label ID="ArticleAuthor" runat="server"   Text='<%#Membership.GetUser(Eval("UserIDfk")) %>' />&#1606;&#1608;&#1740;&#1587;&#1606;&#1583;&#1607;:
            <br />
            <asp:Image ID="ArticleImage" Width="70" Height="70" ImageUrl='<%# Eval("[PostPicture]") %>' runat="server" />
            <br />
            <asp:Label ID="ArticleDate" runat="server" Text='<%# Eval("PostDate") %>' ForeColor="Gray" />
            <br />
            <asp:Label ID="ArticleSummary" runat="server" Text='<%# Eval("ArticleSummary") %>' />
                <br />
                <br />
            </span>
        </ItemTemplate>
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
        ...
        SelectCommand="SELECT Post.PostID, Post.PostType, Post.PostTitle, Post.PostDate, Post.PostPicture, Article.UserIDfk, Article.ArticleSummary, Article.ArticleVisible FROM Article INNER JOIN Post ON Article.PostIDfk = Post.PostID AND Article.PostIDfk = Post.PostID WHERE (Article.ArticleVisible = 1) ORDER BY Post.PostID DESC">
    </asp:SqlDataSource>
